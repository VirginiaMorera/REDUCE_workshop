...
field <- terra::rast(nrows = length(xseq), ncol = length(yseq), vals = rawDat, 
                     xmin = 0, xmax = 1, ymin = 0, ymax = 1)

plot(field)

sampleData <- data.frame(value = sampData[[2]], 
                         x = sampData[[1]][,1], 
                         y = sampData[[1]][,2])


sampleData_sf <- sampleData %>% 
  st_as_sf(coords = c("x", "y")) %>% 
  st_set_crs(crs(field))

library(tidyterra)

ggplot() +
  geom_spatraster(data = field) +
  geom_sf(data = sampleData_sf)
